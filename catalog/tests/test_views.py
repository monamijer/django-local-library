...
from django.contrib.auth.models import Permission #Required to grant the permision needed to set a book as returned
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        ...
        test_user2 = User.objects.create_user(username='testuser2', password='2HJ<aeXW4&g9L')
        test_user1.save()
        test_user2.save()
    ...
    def test_only_borrowed_books_in_list(self):
        ...
        # Check that now we have borrowed books
        response = self.client.get(reverse('my-borrowed'))
        # Check our user is logged in
        self.assertEqual(str(response.context['user']), 'testuser1')
        # Check that we got a response "success"
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_logged_in_with_permission_borrowed_book(self):
        login = self.client.login(username='testuser2', password='2HJ<aeXW4&g9L')
        response = self.client.get(reverse('renew-book-librarian', kwargs={'pk': self.test_bookinstance2.pk}))

        # Check that it lets us login - this means we have the correct permission
        self.assertEqual(response.status_code, 200)
